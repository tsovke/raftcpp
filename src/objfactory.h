...
namespace raft {
template <typename T>
class objfactory : public std::enable_shared_from_this<objfactory<T>>,
                   noncopyable {
  std::mutex m_mutex;
...
public:
  template <typename... Args> std::shared_ptr<T> Get(int id, Args &&...args) {
    std::unique_lock<std::mutex> _(m_mutex);
    std::weak_ptr<T> &ptr = m_map[id];
    std::shared_ptr<T> ret = ptr.lock();
    if (!ret) {
      ret.reset(new T(id, std::forward<Args>(args)...),
                std::bind(&objfactory::DeleteObj, this->shared_from_this(),
                          std::placeholders::_1));
      ptr = ret;
      m_set.insert(id);
    }
