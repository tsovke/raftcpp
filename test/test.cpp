...
int main() {
  ...
  tpool.submit([print] { print->Print(); });

  const auto &MAX_SERVER = 5;

  // 服务器启动，选举出一个leader
  print->AddPrint("\n\nTest->ALL server Start, server_count:" +
                  std::to_string(MAX_SERVER));
  ...
  // 领导掉线，重新选举
  print->AddPrint("\n\nTest->Server:" + std::to_string(leader1) +
                  " DISCONNECT");
  ...
  // 超过一半的服务器掉线，无法完成选举
  print->AddPrint("\n\nTest->Server:" + std::to_string(leader2) +
                  " And More Than Half Server DISCONNECT");
  ...
  // 超过半数的服务器掉线，无法完成日志同步
  print->AddPrint("\n\nTest->ADD LOG");
  factory->Get(leader2, factory)->AddLog("test_2");
  factory->Get(leader2, factory)->AddLog("test_3");
  std::this_thread::sleep_for(std::chrono::seconds(10));
  CheckApplyLog(factory);
